...
sed -ie "
    s~^# \?command_warnings \?= .*~command_warnings = False~
    s~^# \?control_path \?= .*~control_path = %(directory)s/%%h-%%r~
    s~^# \?deprecation_warnings \?= .*~deprecation_warnings = False~
    s~^# \?host_key_checking \?= .*~host_key_checking = False~
    s~^# \?pty \?=.*~pty=False~
    s~^# \?stdout_callback = .*~stdout_callback = skippy~
    s~^# \?system_warnings \?= .*~system_warnings = False~
    s~^# \?timeout \?= .*~timeout = 40~
...
sed -ie "
    s~.{IMAGE_PREFIX}.{IMAGE_BASENAME}:.{IMAGE_VERSION}~djerfy/registry-console~
" /home/$SUDOUSER/openshift-ansible/roles/openshift_hosted_templates/files/v3.9/origin/registry-console.yaml
